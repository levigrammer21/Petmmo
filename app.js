...
const stateUi={screen:'home',skill:'woodcutting',selectedCombat:new Set(),combatKeeper:true,region:'greenhollow',combatStyle:'melee',autoEat:true,autoTame:true,captureItemId:'',cloud:false,user:null,lastRender:0};
...
const elapsed=ms=>{let s=Math.max(0,Math.floor(ms/1000));const d=Math.floor(s/86400);s%=86400;const h=Math.floor(s/3600);s%=3600;const m=Math.floor(s/60);s%=60;if(d)return `${d}d ${h}h`;if(h)return `${h}h ${m}m`;if(m)return `${m}m ${s}s`;return `${s}s`;};
...
function art(species,cls='pet-art'){const src=esc(species.art);return `<img class="${cls}" src="${src}" alt="${esc(species.name)}" loading="lazy" decoding="async" onerror="if(!this.dataset.retry){this.dataset.retry='1';this.src='${src}?v=${GAME_VERSION}'}else{this.remove()}"/>`;}
function keeperAvatar(cls='keeper-avatar'){return `<div class="${cls}" aria-label="Keeper portrait"><svg viewBox="0 0 96 96" aria-hidden="true"><defs><linearGradient id="cloakG" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#276b5f"/><stop offset="1" stop-color="#102e2c"/></linearGradient><linearGradient id="skinG" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#f2c99b"/><stop offset="1" stop-color="#c88d63"/></linearGradient></defs><path class="ka-bg" d="M9 91c2-26 16-41 39-41 24 0 38 15 39 41H9Z"/><path class="ka-cloak" d="M11 94c3-24 16-36 37-36 22 0 35 12 37 36H11Z" fill="url(#cloakG)"/><path class="ka-neck" d="M39 51h19l4 14-14 9-14-9 5-14Z"/><path class="ka-face" d="M28 30C28 14 36 5 49 5c14 0 22 9 22 25 0 18-9 30-22 30-13 0-21-12-21-30Z" fill="url(#skinG)"/><path class="ka-hair" d="M27 30C26 12 36 2 50 2c14 0 23 10 23 27-8-8-15-13-26-11-9 1-14 6-20 12Z"/><path class="ka-hair-side" d="M28 27c-4 10-2 22 6 29l6-5c-5-8-5-16-2-24l-10 0Zm43-3c5 11 3 23-5 32l-6-6c5-8 5-16 1-24l10-2Z"/><path class="ka-brow" d="M36 32c4-2 8-2 11 0m5 0c4-2 8-2 11 0"/><ellipse class="ka-eye" cx="42" cy="37" rx="2.2" ry="2.7"/><ellipse class="ka-eye" cx="58" cy="37" rx="2.2" ry="2.7"/><path class="ka-nose" d="M50 39l-2 8 4 1"/><path class="ka-mouth" d="M42 52c5 3 11 3 16 0"/><path class="ka-scar" d="M62 34l-6 11"/><path class="ka-collar" d="M34 62l14 12 15-12 11 28H22l12-28Z"/><circle class="ka-clasp" cx="48" cy="74" r="4"/><path class="ka-leaf" d="M48 70c5-7 11-7 14-4-1 5-6 8-14 8v-4Z"/></svg></div>`;}
...
function actionBar(a){if(!a)return'';const int=a.intervalMs||5000,start=a.startedAt||Date.now();return `<div class="action-progress"><span data-action-progress data-started-at="${start}" data-interval-ms="${int}"></span></div><small class="running-time" data-running-since="${start}">Running ${elapsed(Date.now()-start)}</small>`;}
...
function processingQueuePanel(){
  ...
  return `<section class="processing-board"><div class="processing-summary"><div><small>REMAINS WAITING</small><strong>${state.remains.length}</strong></div><div><small>PROCESSORS</small><strong>${workers.length+(keeper?1:0)}</strong></div><div><small>COMPLETED</small><strong>${fmt(state.stats.processed)}</strong></div></div>
    ...
    <div><h3>Working now</h3><div class="processor-list">${keeper?`<div class="processor-row">${keeperAvatar()}<span><b>Keeper</b><small>~10s per remain · <span data-running-since="${keeper.startedAt||Date.now()}">Running ${elapsed(Date.now()-(keeper.startedAt||Date.now()))}</span></small></span></div>`:''}${workers.map(({p,a})=>{const sp=SPECIES_BY_ID[p.speciesId];return `<div class="processor-row"><div class="tiny-art">${art(sp)}</div><span><b>${esc(p.customName||sp.name)}</b><small>Processing aptitude ${sp.aptitudes?.processing||1} · ${time(a.intervalMs||10000)} each · <span data-running-since="${a.startedAt||Date.now()}">Running ${elapsed(Date.now()-(a.startedAt||Date.now()))}</span></small></span></div>`;}).join('')||(!keeper?'<div class="empty compact">No one assigned yet.</div>':'')}</div></div></div></section>`;
}
function taskCard(skill,t,level){
  ...
  const activeTimes=[];if(keeperHere)activeTimes.push(`Keeper <span data-running-since="${state.keeperAssignment.startedAt||Date.now()}">${elapsed(Date.now()-(state.keeperAssignment.startedAt||Date.now()))}</span>`);for(const wid of workerIds){const wa=state.petAssignments[wid];const wp=state.pets.find(p=>p.id===wid);if(wa&&wp)activeTimes.push(`${esc(wp.customName||SPECIES_BY_ID[wp.speciesId].name)} <span data-running-since="${wa.startedAt||Date.now()}">${elapsed(Date.now()-(wa.startedAt||Date.now()))}</span>`);}
  return `<article class="task ${locked?'locked':''}"><div class="task-top"><div><small>LEVEL ${t.level} · ${t.duration||10}s base · MASTERY ${mastery}/99</small><h3>${esc(t.name)}</h3></div><span class="reward">${esc(rewardText)}</span></div>${costText?`<p class="cost">Uses: ${esc(costText)}</p>`:''}
    <div class="task-actions"><button class="btn ${keeperHere?'danger':'primary'}" data-keeper-task="${type}|${type==='processing'?'remains':t.id}" ${locked?'disabled':''}>${keeperHere?'Stop Keeper':'Train this'}</button><button class="btn" data-pet-picker="${type}|${type==='processing'?'remains':t.id}" ${locked?'disabled':''}>Assign pet</button>${workerIds.length?`<span class="worker-chip">${workerIds.length} pet${workerIds.length===1?'':'s'} working</span>`:''}</div>${activeTimes.length?`<small class="task-running">Running: ${activeTimes.join(' · ')}</small>`:''}</article>`;
}
function renderPets(){
...
function renderCombat(){
  ...
  return `${pageHead('CONTINUOUS COMBAT','The Wilds','Pick a region, party, combat style, and taming offering. Battles chain continuously with no search timer.')}
    <section class="combat-setup-card"><div class="combat-options"><label>Region<select id="combat-region" class="input">${REGIONS.map(r=>`<option value="${r.id}" ${stateUi.region===r.id?'selected':''}>${r.name} · Combat ${r.level}+</option>`).join('')}</select></label><label>Keeper style<select id="combat-style" class="input"><option value="melee" ${stateUi.combatStyle==='melee'?'selected':''}>Melee</option><option value="ranged" ${stateUi.combatStyle==='ranged'?'selected':''}>Ranged</option><option value="magic" ${stateUi.combatStyle==='magic'?'selected':''}>Magic</option></select></label></div>
      <div class="toggle-row"><label class="check"><input id="combat-keeper" type="checkbox" ${stateUi.combatKeeper?'checked':''}/> Keeper fights</label><label class="check"><input id="combat-autoeat" type="checkbox" ${stateUi.autoEat?'checked':''}/> Auto-eat</label><label class="check"><input id="combat-autotame" type="checkbox" ${stateUi.autoTame?'checked':''}/> Auto-tame</label></div>
      <div class="taming-box"><div><b>🐾 Taming</b><small>After a victory, Auto-tame uses one offering and rolls the species' tame chance. Better food and Pet Mastery improve the odds.</small></div><label>Offering<select id="combat-offering" class="input"><option value="" ${!stateUi.captureItemId?'selected':''}>No offering</option>${offerings.map(([id,q])=>`<option value="${id}" ${stateUi.captureItemId===id?'selected':''}>${itemName(id)} ×${q} · +${Math.round((ITEMS[id].captureBonus||0)*100)}%</option>`).join('')}</select></label></div>
...
function renderActiveCombat(){const c=state.combat,e=c.enemy,sp=e?SPECIES_BY_ID[e.speciesId]:null;const region=REGIONS.find(r=>r.id===c.regionId);const party=c.petIds.map(id=>state.pets.find(p=>p.id===id)).filter(Boolean);const offer=c.captureItemId?ITEMS[c.captureItemId]:null;const tame=sp?captureChanceFor(state,sp.id,c.autoTame?c.captureItemId:''):0;const events=[...(c.events||[])].reverse().slice(0,7);
  return `${pageHead(c.dungeonId?'DUNGEON FIGHT':'LIVE COMBAT',c.dungeonId?(DUNGEONS.find(d=>d.id===c.dungeonId)?.name||'Dungeon'):region?.name||'The Wilds',c.state==='stopped'?'Your party is down. Heal them and resume when ready.':'Attacks, crits, affinities, abilities, taming, and the next enemy all resolve live.',`<button id="stop-combat" class="btn danger">Stop combat</button>`)}
    <section class="battle-stage ${c.state==='stopped'?'battle-stopped':''} ${c.lastEvent?.kind||''}"><div class="battle-status"><span class="live-dot"></span><b>${c.state==='between'?'Next enemy…':c.state==='stopped'?'Party down':'FIGHTING'}</b><small>Battle ${c.battle||1} · Streak ${c.streak||0} · Best ${state.stats.bestStreak||0} · <span data-running-since="${c.startedAt||Date.now()}">Running ${elapsed(Date.now()-(c.startedAt||Date.now()))}</span></small></div>
...
function bindScreen(){
  ...
  if($('#combat-style'))$('#combat-style').onchange=e=>{stateUi.combatStyle=e.target.value;};
  if($('#combat-autoeat'))$('#combat-autoeat').onchange=e=>{stateUi.autoEat=e.target.checked;};
  if($('#combat-autotame'))$('#combat-autotame').onchange=e=>{stateUi.autoTame=e.target.checked;};
  if($('#combat-offering'))$('#combat-offering').onchange=e=>{stateUi.captureItemId=e.target.value;};
  if($('#combat-keeper'))$('#combat-keeper').onchange=e=>{stateUi.combatKeeper=e.target.checked;updatePower();};
  $$('[data-combat-pet]').forEach(cb=>cb.onchange=()=>{if(cb.checked){if(stateUi.selectedCombat.size>=MAX_COMBAT_PETS){cb.checked=false;toast(`Choose up to ${MAX_COMBAT_PETS} pets.`);return;}stateUi.selectedCombat.add(cb.value);}else stateUi.selectedCombat.delete(cb.value);cb.closest('.party-pick')?.classList.toggle('selected',cb.checked);updatePower();});
  if($('#start-combat'))$('#start-combat').onclick=()=>{try{setState(startCombat(state,{regionId:$('#combat-region').value,petIds:[...stateUi.selectedCombat],includeKeeper:$('#combat-keeper').checked,style:stateUi.combatStyle,autoEat:stateUi.autoEat,autoTame:stateUi.autoTame,captureItemId:stateUi.captureItemId}),'Combat started.');}catch(e){toast(e.message);}};
...
function uiIsBusy(){const a=document.activeElement;return Boolean($('#modal')?.open || (a&&['SELECT','INPUT'].includes(a.tagName)));}
function updateHeaderOnly(){if(!state)return;$('#coins').textContent=fmt(state.profile.coins);$('#keeper-hp').textContent=`${Math.ceil(state.profile.currentHp)}/${state.profile.maxHp}`;$('#pet-count').textContent=`${state.pets.length}/${denCapacity(state)}`;}
function animateUi(){if(state&&!$('#game').classList.contains('hidden')){const now=Date.now();$$('[data-action-progress]').forEach(el=>{const started=Number(el.dataset.startedAt||now),int=Math.max(1,Number(el.dataset.intervalMs||5000));el.style.transform=`scaleX(${((now-started)%int)/int})`;});$$('[data-running-since]').forEach(el=>{const started=Number(el.dataset.runningSince||now);el.textContent=`Running ${elapsed(now-started)}`;});}requestAnimationFrame(animateUi);}
let lastPassiveRefresh=0;
setInterval(()=>{
  if(!state||$('#game').classList.contains('hidden'))return;
  const now=Date.now();
  if(state.combat&&stateUi.screen==='combat'&&!document.hidden){state=advanceLive(state,now);saveSoon();if(!uiIsBusy()&&now-stateUi.lastRender>450)render();}
  else {state=settleState(state,now);updateHeaderOnly();if(stateUi.screen!=='combat'&&!uiIsBusy()&&now-lastPassiveRefresh>5000){lastPassiveRefresh=now;render();}}
},250);
requestAnimationFrame(animateUi);
init();
